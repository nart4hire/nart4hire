...
async function seed() {
  ...
  try {
    ...
    const existingUser = await db
      .select()
      .from(users)
      .where(eq(users.email, 'business@nathancs.dev'))
      .get()

    if (existingUser) {
      console.log('✓ Admin user already exists')
      console.log(`  Email: business@nathancs.dev`)
      console.log(`  Role: ${existingUser.role}`)
      return
    }

    // Hash password using SHA-256 (same as SonicJS auth system)
    const data = 'masterof@12' + 'salt-change-in-production'
